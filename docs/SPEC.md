...
| 項目 | 内容 |
|------|------|
| トリガ | **Google Apps Script（GAS）の時間主導トリガ**: **毎日 8:00（JST）前後**に朝の軽量プッシュ（`sendMorningMessage`）、**毎日 17:40（JST）前後**に日次集計プッシュ、**毎週土曜 8:00〜8:59（JST）前後**に週次（§1.4）。TZ は **`Asia/Tokyo` 等で明示**する。 |
...
| 15 | 朝のプッシュ | **毎日 8:00（JST）前後**に `sendMorningMessage`（テンプレのみ・Gemini なし）。`installMorningMessageTrigger` で登録。揺らぎは §1.1.1。**土曜**は §1.4 W-2 の週次と **同一時台**になりうるが、べき等キーは `morning:` と `sent_week:` で分離され **両方届きうる** |
...
| W-2 | 配信時刻 | **土曜 8 時台（8:00〜8:59 JST）**。GAS `atHour(8)` による 1 時間幅。揺らぎは §1.1.1 と同様に許容する。**§1.3 #15** の朝プッシュも毎日 8 時台のため、土曜は **朝の一声と週次の 2 プッシュ**が近い時間に届くことがある |
...
| W-8 | LIFF エンドポイント | **日次と同一の `doGet`**。`?weekStart=YYYY-MM-DD&token=xxx` クエリで日次（`?date=...`）と区別する（§5.7.3）。実装では `parseWebAppQuery_` で URL 直下と `liff.state` をマージし、週次トークンエラーは `weeklyTokenErrorHtmlHint_` を用いる。HTML は LIFF 内表示向けに `XFrameOptionsMode.ALLOWALL` を付与する |
...
flowchart TB
  subgraph trigger [トリガ（GAS 時間主導・JST）]
    T_AM[毎日 ~8:00: sendMorningMessage]
...
- **URL 例**: `https://script.google.com/...?weekStart=2026-04-13&token=xxx`
- **weekStart**: その週の月曜の JST 日付（`YYYY-MM-DD`）
- **トークン保存キー**: `yattakai_week_token_{weekStartJst}`（スクリプトプロパティ）
- **`doGet` 分岐**: `weekStart` パラメータの有無で日次・週次を判定する（`date` 優先。両方あれば日次）
- **有効期限**: 付録 A.2 の 90 日保持ポリシーに従いクリーンアップする
...
`sent:YYYY-MM-DD` キーと `yattakai_dash_token_YYYY-MM-DD` キーは GAS ScriptProperties に蓄積する。週次機能追加後は `sent_week:YYYY-MM-DD`（月曜日付）と `yattakai_week_token_YYYY-MM-DD` も同様に蓄積するため、クリーンアップ対象に含める。プロパティ数の上限（500 件）に達するのは日次・週次合算でも十分な余裕があるが、定期削除で安全に運用する。

- **削除対象プレフィックス**: `sent:` / `yattakai_dash_token_` / `sent_week:` / `yattakai_week_token_`
- **保持日数**: 90 日（デフォルト。変更可）
