...
`sent:YYYY-MM-DD` キーと `yattakai_dash_token_YYYY-MM-DD` キーは GAS ScriptProperties に蔓積する。プロパティ数の上限（500 件）に達するのは約 500 日後のため、**週次トリガで定期削除**する。

- **保持日数**: 90 日（デフォルト。変更可）
- **実行タイミング**: 毎週月曜 3:00（JST）を推奨
