...
| 処理 | タスクを正規化し、**達成率・カテゴリ別集計**を算出。■2（名言パート）は **MVP では外部 AI を使わず**、**名言リスト＋テンプレートでイチさん／ヒロ子の台詞を組み立てる**（詳細は §6）。 |
...
| # | テーマ | 決定内容 |
|---|--------|----------|
| 1 | 実行形態 | **GAS 中心**（まず A）。時間主導トリガ・Sheets・`UrlFetchApp` で **LINE**。**MVP は外部 AI API を呼ばない** |
...
| 9 | ■2（名言・セリフ） | **外部 AI は使わない**。名言は **`Quotes` シート（§4.4）または GAS 内の定義リスト**から、**日付等で決定的に 1 件選ぶ**。イチさん／ヒロ子の文は **固定テンプレート＋変数差し込み**（名言・達成率・§3.3 メッセージ等） |
| 10 | 公開 URL 対策 | ウェブアプリを **匿名アクセス可**にするため、プッシュに載せる URL に **その日だけ有効なランダム `token`** を付与し、`date` と組で GAS 側検証する |
| 11 | ■2 データ欠落時 | **LINE プッシュは送る**。■2 は **固定の短いフォールバック文**（例: 本日の名言データがない）＋■1 系は表示可能なら表示 |
| 12 | ソース管理 | **`clasp`** でローカル（`yattakai-task-checker`）と双方向同期し **Git にコミット** |
| 13 | ウェブアプリデプロイ | **実行ユーザー**: スクリプト所有者。**アクセス**: **全員（匿名を含む）** → §1.3 の **トークン検証が必須** |
| 14 | 将来の AI | **Anthropic 等は後から**。`AiDialogue`（§2.1）で §6.1 のテンプレ経路を置き換え可能にする |
...
- （**AI を導入する段階**になったら）API キー等。**MVP では不要**
...
「スプレッドシート」「LINE」「■2 台詞生成（MVP はテンプレ／将来は AI 可）」を **ドメインモデル**で切り離す。
...
flowchart TB
  ...
  subgraph core [コア]
    ...
    SCORE[達成率% と閾値メッセージ]
    QUOTE[MVP: Quotes から決定選択 + テンプレ台詞]
  end
...
| **DialogueBuilder（MVP）** / **AiDialogue（将来）** | イチ／ヒロの台詞組み立て | **MVP**: テンプレ＋差し込み。**将来**: 外部 LLM |
...
外部 AI を使わない ■2 用。**手入力で十分**ならシート推奨（編集が容易）。
...
- **イチさん**: 名言を読み上げる口調（**MVP**: テンプレート文）
- **ヒロ子**: 意味の簡単な解説 + リアクション（**MVP**: テンプレート文）
- **素材（MVP）**: **`Quotes` シート（§4.4）または GAS 内配列**。**決定的な 1 件選択**（例: 日付ハッシュでインデックス、または達成率帯ごとに別リスト）
- **トーン（MVP）**: **テンプレート**を **§3.3 の閾値メッセージ**や達成率に合わせて切り替える（AI なし）
- **著作権・出典**: **MVP は `Quotes` に自分が権利を確認した文言だけ**を載せる運用が簡潔。将来ネット収集する場合は `docs/` にポリシーを置く
...
| 出力 | **`{ quote, ichisan, hiroko }` 形式の JSON** を GAS で組み立て（将来 AI 応答と同形にすると差し替え容易） |

**バリデーション**: `Quotes` 行は **必須列・空 text** を検査。不正行はスキップしログ。有効行が 0 件なら §1.3 #11 のフォールバック。

### 6.2 将来（AI 再導入時・保留）

課金・運用が整ったら **`AiDialogue` 実装**（§2.1）に切り替え。

- 名言は引き続き **リスト起点**とし、AI は **言い回し**や **候補選択**に限定すると安全
- プロンプトは **XML タグ**（`<reference_quote>`, `<user_achievement>`）で入力分離、**Few-shot**、**JSON 出力**を要求
- API 失敗時は **§6.1 のテンプレ経路へ自動フォールバック**を推奨
...
3. **AI 再導入時（§6.2）**: XML / few-shot / 出力 JSON スキーマの固定版、**リトライ・バックオフ**、§6.1 へのフォールバック
