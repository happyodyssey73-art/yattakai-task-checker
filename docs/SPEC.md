...
- ~~LIFF からの **即時**シート書き戻し~~　← **実装済み**（`updateTaskStatus`・§5.1.1）
...
#### 3.3.1 朝の自動行投入と 0% 挑戦トーン（確定）

`ensureDailyRowsForToday_` が朝・夕に active タスク分の `Daily` 行を **`×`（not_done）** で追記する。ユーザーが LIFF で ◯/✕ をタップする前でも、**達成率は 0%** となり、§3.3.2 の最低帯メッセージ（**「まずははじめの一歩」**）が適用される（挑戦トーン）。

| 観点 | ルール |
|------|--------|
| 集計 | `×` は分母に含め、分子には含めない（§3.2） |
| LIFF 行 UI | `updated_at` が空の行は **`is_explicit=false`** → ニュートラル表示（赤 `×` に見せない） |
| ユーザー操作後 | `updated_at` を付与し **`is_explicit=true`** → ◯ 緑 / ✕ 赤で明示 |

「まだ記入されていません」等の **中立メッセージは使わない**（2026-06 確定）。
...
17:40 の日次 LIFF は **締め作業モード（§5.1.1）** を採用する。LINE 通知で達成率・セリフを既に読んでいる前提で、LIFF では **◯/✕ 記録を最優先**する。

#### 5.1.1 日次 LIFF・締め作業モード（17:40・確定）

ファーストビューから下へ、次の順で固定する（タスクは **折りたたまない**）:

1. **日付**（小さく）
2. **操作ヒント**（例: ◯／✕ をタップして、今日の記録を締めましょう）
3. **今日のタスク**（全件・◯/✕ ボタン）
4. **今日の結果**（ドーナツ＋達成率％＋§3.3 メッセージ＋件数）
5. **カテゴリ別**バー
6. **■2 今日の一言**（記録後の感情報酬）

タスク更新後は **結果ブロック（％・mood・ドーナツ・カテゴリ）とキャラ表情** を差分更新する。セリフ全文の再生成は不要（`skipSection2` 経路）。

#### 5.1.2 参照用（旧ファーストビュー案）

振り返り専用画面を別途作る場合の参考: 達成率 → mood → ■2。現行日次 LIFF では **§5.1.1 を正**とする。
...
- **v1**: LIFF から ◯/✕ で **即時書き戻し**（`updateTaskStatus`）。`onTaskToggle` で楽観的更新
...
| 2026-06-09 | **日次 LIFF 締め作業モード（§5.1.1）**: タスク最上 → 結果サマリー → カテゴリ → ■2。**§3.3.1** を 0% 挑戦トーンに改訂。LIFF 書き戻し実装済みを §1.2・§5.3 に反映。ユーザー向けエラー文言一元化（`dashboardClientErrorMessage_`） |